...
def converge_interior_wall(slices, alpha_env, T_env):
    ...
    while True:
        for i in range(len(start_slices)):
            ...
            start_slices[i].T_interior_wall = 1.5*delta_T + old_temp
        ...
        for i, slice in enumerate(iteration_slices[:-1]):
            ...
            alpha_int = iteration_slice.alpha_interior_fluid
            A_int = iteration_slice.lateral_area_interior_fluid
            T_int = iteration_slice.T_interior_fluid
            ...
            while True:
                ...
                if i == 0:
                    T_w = 0
                    aw = 0
                    lambda_e = geometric_avg(east_slice.lambda_interior_wall, iteration_slice.lambda_interior_wall, east_slice.dx, iteration_slice.dx)
                    ae = lambda_e * A_e / d_e
                    
                else:
                    west_slice = iteration_slices[i-1]
                    lambda_w = geometric_avg(west_slice.lambda_interior_wall, iteration_slice.lambda_interior_wall, west_slice.dx, iteration_slice.dx)
                    T_w = west_slice.T_interior_wall
                    aw = lambda_w * A_w / d_w

                    if i ==len(iteration_slices) - 1:
                        T_e = iteration_slice.T_interior_fluid
                        alpha_e = iteration_slice.alpha_interior_fluid
                        ae = alpha_e * T_e
                ...
                iteration_slice.T_interior_wall = (aw * T_w + ad * T_int + au * T_ext + ae * T_e) / (aw + ad + ae + au)
